...
export function ProgressTimeline({ progressList, onEdit, onShare, onPin, victoryNumbersById }: Props) {
  const progressWithNumbers = useMemo(() => {
    if (victoryNumbersById && Object.keys(victoryNumbersById).length > 0) {
      return progressList.map((progress) => ({
        progress,
        victoryNumber: victoryNumbersById[progress.id] ?? 0,
      }));
    }
  ...
  }, [progressList, victoryNumbersById]);
  ...
  return (
    <div className="relative">
      ...
      <div className="flex flex-col gap-6 md:gap-10 relative z-20 pt-4 md:pt-6">
        {progressWithNumbers.map(({ progress, victoryNumber }) => (
